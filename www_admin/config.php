...
function perform(&$msg) {
  ...
  if ($_POST["admin_username"] && $_POST["admin_password"]) {
    ...
    // Previously, the password was created blindly using Crypt algorithm, which is not
    // supported on all *nix platforms Apache installations. MD5 is well-supported.
    // This should really be improved by letting the WuHu admin to select an option
    // from a dropdown.
    $htpasswd = new Htpasswd('.htpasswd');
    $htpasswd->addUser($_POST["admin_username"], $_POST["admin_password"], Htpasswd::ENCTYPE_APR_MD5);
  }
...
function getRelativeProposedDir($dirName, $defaultPath = "/var/www/") {
  $directory = realpath(getcwd() . "/../" . $dirName);
  $directory = $directory === FALSE ? $defaultPath . $dirName : $directory;
  return $directory;
}

$directory_www_party = getRelativeProposedDir("www_party");
$directory_entries_private = getRelativeProposedDir("entries_private");
$directory_screenshots = getRelativeProposedDir("screenshots");
$directory_export = getRelativeProposedDir("export");
    ...
    <tr>
      <td>What is the <b><u>absolute</u></b> path of the directory
        ...
        <small>(<b>Optional</b> - This is a helper directory; you can either use this to
          ...
          share the files with the visitors or to upload to scene.org. Should have read/write permissions for Apache.)
        </small>
      </td>
      <td>
    ...
    <tr>
      <td>What is the <b><u>absolute</u></b> path of the directory
        ...
        <small>(This should be a directory with read/write permissions for Apache,
          but it doesn't have to be accessible for anyone else.)
        </small>
      </td>
      <td>
