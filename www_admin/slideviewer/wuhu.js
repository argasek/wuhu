var WuhuSlideSystem = Class.create({
    ...
    reLayout: function () {
        $$('.reveal .slides>section').each(function (item) {
            ...
            if (cont) {
                var value = ((this.revealOptions.height - cont.getLayout().get("height")) / 2);
        ...
        $A(this.slides).each(function (slide) {
            ...
            switch (ext) {
                ...
                case "mp4":
                {
                    ...
                    video.observe("load", (function () {
                        this.reLayout();
                    }).bind(this));
                    video.observe("loadedmetadata", (function () {
                        this.reLayout();
                    }).bind(this));
    ...
    updateCountdownTimer: function () {
        ...
        if (this.options.showHours) {
            s = ("000" + (sec % 60)).slice(-2);
            sec = Math.floor(sec / 60);
            s = ("000" + (sec % 60)).slice(-2) + ":" + s;
            sec = Math.floor(sec / 60);
            s = ("" + (sec)) + ":" + s;
        }
        else {
        ...
        new Ajax.Request("../result.xml?" + Math.random(), {
            ...
            onSuccess: (function (transport) {
                ...
                switch (mode) {
                    case "announcement":
                    {
                        sec = this.insertSlide({"class": "announcementSlide eventSlide"});
                        cont = sec.down("div.container");
                        cont.insert(new Element("div", {"class": "wrapper"}));
                        cont = cont.down("div.wrapper");
                    ...
                    case "compocountdown":
                    {
                        sec = this.insertSlide({"class": "countdownSlide eventSlide"});
                        cont = sec.down("div.container");
                        cont.insert(new Element("div", {"class": "wrapper"}));
                        cont = cont.down("div.wrapper");
                        ...
                        cont.insert(new Element("div", {"class": "eventName textLarge"}).update(openingText));
                        cont.insert(new Element("div", {"class": "isStartingIn textMedium"}).update("will start in"));
                        cont.insert(new Element("div", {"class": "countdownTimer textMedium"}).update("0"));
                    ...
                    case "compodisplay":
                    {
                        ...
                        sec = this.insertSlide({"class": "compoDisplaySlide eventSlide intro"});
                        cont = sec.down("div.container");
                        cont.insert(new Element("div", {"class": "wrapper"}));
                        cont = cont.down("div.wrapper");
                        cont.insert(new Element("div", {"class": "eventName textLarge"}).update(compoNameFull));
                        cont.insert(new Element("div", {"class": "willStart textMedium"}).update("will start"));
                        cont.insert(new Element("div", {"class": "now textMedium"}).update("now!"));
                        ...
                        sec = this.insertSlide({"class": "compoDisplaySlide eventSlide outro"});
                        cont = sec.down("div.container");
                        cont.insert(new Element("div", {"class": "wrapper"}));
                        cont = cont.down("div.wrapper");

                        cont.insert(new Element("div", {"class": "eventName textLarge"}).update(compoNameFull));
                        cont.insert(new Element("div", {"class": "is textMedium"}).update("is"));
                        cont.insert(new Element("div", {"class": "over textMedium"}).update("over!"));
                    ...
                    case "prizegiving":
                    {
                        ...
                        // slide 1: introduction
                        sec = this.insertSlide({"class": "prizegivingSlide eventSlide intro"});
                        cont = sec.down("div.container");
                        cont.insert(new Element("div", {"class": "wrapper"}));
                        cont = cont.down("div.wrapper");
                        cont.insert(new Element("div", {"class": "header textHuge"}).update("Prizegiving"));
                        cont.insert(new Element("div", {"class": "eventName textMedium"}).update(compoName));

                        // slide 2..n: entries

                        Element.select(e, "result > results entry").each(function (entry) {
                            var sec = this.insertSlide({"class": "prizegivingSlide eventSlide entry"});
                            var cont = sec.down("div.container");
                            cont.insert(new Element("div", {"class": "wrapper"}));
                            cont = cont.down("div.wrapper");
                            var fields = ["ranking", "author", "title", "points"];
                            var fieldsClasses = ['textMedium', 'textLarge', 'textMedium', ''];
                            var i = 0;
                            fields.each(function (field) {
                                if (Element.down(entry, field)) {
                                    var s = Element.down(entry, field).innerHTML;
                                    ...
                                    cont.insert(new Element("div", {"class": field + ' ' + fieldsClasses[i]}).update(s));
                                    i++;
                                }
                            }, this);
                            //cont.insert(new Element("div", {"class": "eventName"}).update(compoName));
    ...
    initialize: function (opt) {
        ...
        document.observe("keyup", (function (ev) {
            if (ev.keyCode == ' '.charCodeAt(0)) {
                document.body.classList.remove('mode-rotation');
                this.slideMode = this.MODE_EVENT;
                this.fetchSlideEvents();
                ev.stop();
            }
            if (ev.keyCode == 'S'.charCodeAt(0)) {
                document.body.classList.add('mode-rotation');
            ...
            switch (ev.keyCode) {
                ...
                case Event.KEY_HOME:
                    ...
                    break;
                case Event.KEY_END:
                    Reveal.slide($$('.reveal .slides>section').length - 1);
                    ev.stop();
                    break;
                    // Argasek: no, we don't want this ;)
                //case Event.KEY_ESC:
                //{
                //    ev.stop();
                ...
                //{
                //    ev.stop();
                //    if (Reveal.isOverview()) Reveal.toggleOverview();
                //}
                    break;
            }
...
var WuhuSlideSystemCanvas = Class.create(WuhuSlideSystem, {
    ...
    initialize: function ($super, options) {
        $super(options);
        this.canvases = [];
        this.sourceCanvas = new Element("canvas", {
            width: options.width,
